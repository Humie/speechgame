...
void CAVPlayer::Timetopause(int pos)
{
	float nowpos = GetPos() + pos;
	libvlc_media_player_play(m_pVLC_Player);
	while (1) {
		float p = GetPos();
		if (p == nowpos) {
...
{   
    if (m_pVLC_Player)
    {
		float totalsec = libvlc_media_player_get_length(m_pVLC_Player) / 1000;
		float position = libvlc_media_player_get_position(m_pVLC_Player);
		float getpos = totalsec * position;
        return getpos;
    }
...
void CAVPlayer::SeekTo(int nPos)
{
    ...
    {
		float totalsec = libvlc_media_player_get_length(m_pVLC_Player) / 1000;
		float tm = nPos / totalsec;
        libvlc_media_player_set_position(m_pVLC_Player, tm);//视频进度的百分比
    }
}
